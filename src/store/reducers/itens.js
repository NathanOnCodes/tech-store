...
const itensSlice = createSlice({
  ...
  reducers: {
    ...
    mostraDescricao: (state, { mostra, payload}) => {
        state.map((item)=> item.mostra !== mostra && item.id === payload ? item.descricao : null)
       return
    }
  }
});

export const { mudarFavorito, mostraDescricao } = itensSlice.actions;
